                ...
                <form action="{{ route('admin.books.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" class="form-control @error('isbn') is-invalid @enderror" 
                               ...
                               placeholder="ISBN (harus unik)" required>
                        @error('isbn')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="number" class="form-control @error('copies') is-invalid @enderror" 
                               id="copies" name="copies" value="{{ old('copies', 1) }}" 
                               min="1" placeholder="Jumlah buku dalam stok" required>
                        @error('copies')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  ...
                                  placeholder="Deskripsi buku">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
