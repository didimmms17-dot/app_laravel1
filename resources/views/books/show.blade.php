...
@if ($errors->any())
    <div style="position: fixed; top: 20px; right: 20px; background: #FEE2E2; border: 2px solid #EF4444; color: #991B1B; padding: 1rem; border-radius: 8px; z-index: 9999;">
        @foreach ($errors->all() as $error)
            <p><x-icon name="x-circle" class="icon-inline" />{{ $error }}</p>
        @endforeach
    </div>
@endif
...
@if (session('status'))
    <div style="position: fixed; top: 20px; right: 20px; background: #DCFCE7; border: 2px solid #22C55E; color: #166534; padding: 1rem; border-radius: 8px; z-index: 9999;">
        <p><x-icon name="check-circle" class="icon-inline" />{{ session('status') }}</p>
    </div>
@endif
...
<div class="book-detail-container">
    <div class="book-detail-wrapper">
        ...
        <div class="breadcrumb">
            <a href="{{ route('welcome') }}"><x-icon name="home" class="icon-inline" />Beranda</a>
            <span class="breadcrumb-separator">/</span>
            <a href="{{ route('books.index') }}"><x-icon name="book" class="icon-inline" />Koleksi Buku</a>
            <span class="breadcrumb-separator">/</span>
        ...
        <div class="book-detail-card">
            <div class="book-detail-content">
                    ...
                    <div class="book-actions-detail">
                        @if(auth()->user() && auth()->user()->role === 'admin')
                            <button class="btn-large btn-disabled-large" disabled>
                                <x-icon name="book" class="icon-inline" />Tidak bisa pinjam
                            </button>
                        @elseif($book->copies > 0)
                            <button class="btn-large btn-borrow-large" onclick="openBorrowModal()">
                                <x-icon name="book" class="icon-inline" />Pinjam Sekarang
                            </button>
                        @else
                            <button class="btn-large btn-disabled-large" disabled>
                                <x-icon name="book" class="icon-inline" />Sedang Habis
                            </button>
                        @endif
                        @auth
                            @if(auth()->user()->role === 'user')
                                <button
                                    id="favoriteBtn"
                                    class="btn-large btn-favorite-large {{ $isFavorited ? 'favorited' : '' }}"
                                    data-book-id="{{ $book->id }}"
                                    type="button"
                                >
                                    <x-icon name="heart" class="icon-inline favorite-icon" />
                                    <span class="favorite-label">{{ $isFavorited ? 'Favorit' : 'Tambah Favorit' }}</span>
                                </button>
                            @endif
                        @else
                            <a class="btn-large btn-favorite-large" href="{{ route('login') }}">
                                <x-icon name="heart" class="icon-inline favorite-icon" />
                ...
                <div class="book-info-section">
                    ...
                    <div class="book-meta">
                        <div class="meta-item">
                            <span class="meta-label"><x-icon name="user" class="icon-inline" />Penulis</span>
                            <span class="meta-value">{{ $book->author ?? 'Tidak Diketahui' }}</span>
                        </div>

                        <div class="meta-item">
                            <span class="meta-label"><x-icon name="tag" class="icon-inline" />Kategori</span>
                            <span>
                                <span class="book-category-large">{{ $book->category->name ?? 'Umum' }}</span>
                            </span>
                        </div>

                        <div class="meta-item">
                            <span class="meta-label"><x-icon name="book-open" class="icon-inline" />Penerbit</span>
                            <span class="meta-value">{{ $book->publisher ?? 'Tidak Diketahui' }}</span>
                        </div>

                        <div class="meta-item">
                            <span class="meta-label"><x-icon name="calendar" class="icon-inline" />Tahun</span>
                            <span class="meta-value">{{ $book->year_published ?? 'Tidak Diketahui' }}</span>
                        </div>

                        <div class="meta-item">
                            <span class="meta-label"><x-icon name="file-text" class="icon-inline" />Halaman</span>
                            <span class="meta-value">{{ $book->pages ?? 'Tidak Diketahui' }} halaman</span>
                        </div>
                    </div>
                    ...
                    <div class="meta-item">
                        <span class="meta-label"><x-icon name="star" class="icon-inline" />Rating</span>
                        <div style="display: flex; align-items: center; gap: 1rem;">
                            <span style="color: #FBBF24; font-size: 1.2rem;"><x-icon name="star" class="icon-inline" />{{ method_exists($book, 'getAverageRating') ? $book->getAverageRating() : ($book->average_rating ?? 0) }}/5</span>
                            <span class="rating-text">({{ method_exists($book, 'getRatingCount') ? $book->getRatingCount() : ($book->rating_count ?? 0) }} ulasan)</span>
                        </div>
                    </div>

                    <div class="meta-item">
                        <span class="meta-label"><x-icon name="info" class="icon-inline" />Ketersediaan</span>
                        <span class="availability-badge {{ $book->copies > 0 ? 'available' : 'unavailable' }}">
                            @if($book->copies > 0)
                                <x-icon name="check-circle" class="icon-inline" />{{ $book->copies }} Buku Tersedia
                            @else
                                <x-icon name="x-circle" class="icon-inline" />Sedang Habis
                            @endif
                        </span>
                    </div>
                    ...
                    <div class="book-description">
                        <h3><x-icon name="file-text" class="icon-inline" />Deskripsi Buku</h3>
                        <p>
                            {{ $book->description ?? 'Buku "' . $book->title . '" karya ' . ($book->author ?? 'Penulis Hebat') . ' adalah sebuah karya yang luar biasa yang akan membawa Anda dalam perjalanan penuh makna dan pembelajaran. Dengan gaya penulisan yang menarik dan plot yang engaging, buku ini menjadi pilihan sempurna untuk pembaca yang mencari inspirasi dan hiburan berkualitas. Nikmati pengalaman membaca yang tak terlupakan dengan koleksi buku kami.' }}
                        </p>
                    </div>
            ...
            <div style="padding: 3rem; border-top: 2px solid #E0E7FF;">
                <div class="section-tabs">
                    <button class="tab-btn active" onclick="switchTab(event, 'detail')">
                        <x-icon name="clipboard" class="icon-inline" />Detail Lengkap
                    </button>
                    <button class="tab-btn" onclick="switchTab(event, 'rating')">
                            <x-icon name="message-circle" class="icon-inline" />Ulasan & Rating ({{ method_exists($book, 'getRatingCount') ? $book->getRatingCount() : ($book->rating_count ?? 0) }})
                        </button>
                </div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 2rem;">
                        <div>
                            ...
                            <div style="display: flex; flex-direction: column; gap: 0.75rem; color: #475569;">
                                <div>
                                    <strong>ISBN:</strong> {{ $book->isbn ?? '978-XXXXXXXXXX' }}
                                </div>
                                <div>
                                    <strong>Bahasa:</strong> {{ $book->language ?? 'Indonesia' }}
                                </div>
                                <div>
                                    <strong>Jenis Buku:</strong> {{ $book->type ?? 'Buku Cetak' }}
                                </div>
                                <div>
                                    <strong>Format:</strong> {{ $book->format ?? 'Hardcover' }}
                                </div>
                            </div>
                        </div>
                        <div>
                ...
                <div id="rating" class="tab-content">
                    ...
                    <div class="rating-form-section">
                        <h4><x-icon name="star" class="icon-inline" />Beri Rating & Ulasan Buku</h4>
                        
                        <form action="{{ route('books.rating', $book->id) }}" method="POST">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <div class="rating-stars" id="ratingStars">
                                    @for($i = 1; $i <= 5; $i++)
                                        <span class="rating-star" onclick="setRating({{ $i }})"><x-icon name="star" /></span>
                                    @endfor
                                </div>
                            ...
                            <div class="form-actions">
                                ...
                                <button type="submit" class="btn-submit">
                                    <x-icon name="check-circle" class="icon-inline" />Kirim Rating
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div style="background: #DBEAFE; border: 2px solid #BFDBFE; border-radius: 12px; padding: 2rem; text-align: center; margin-bottom: 2rem;">
                        <p style="color: #1E3A8A; font-weight: 700; margin-bottom: 1rem;">
                            <x-icon name="lock" class="icon-inline" />Silakan Login untuk memberikan rating
                        </p>
                        <a href="{{ route('login') }}" style="display: inline-block; padding: 0.75rem 1.5rem; background: #2563EB; color: white; text-decoration: none; border-radius: 8px; font-weight: 700;">
                            Login Sekarang
                        </a>
                    </div>
                    @endauth
                    ...
                    <h4 style="color: #1E3A8A; font-weight: 700; margin-bottom: 1.5rem; margin-top: 2rem;">
                        <x-icon name="message-circle" class="icon-inline" />Ulasan Pembaca ({{ method_exists($book, 'getRatingCount') ? $book->getRatingCount() : ($book->rating_count ?? 0) }})
                    </h4>
                                ...
                                <div class="review-item">
                                    <div class="review-header">
                                        <div class="review-avatar">
                                            {{ strtoupper(substr($rating->user->name ?? 'U', 0, 1)) }}{{ strtoupper(substr($rating->user->name ?? 'U', -1, 1)) }}
                                        </div>
                                        <div class="review-info">
                                            <h4>{{ $rating->user->name ?? 'Pembaca Anonim' }}</h4>
                                            <div class="review-date">{{ $rating->created_at->format('d M Y') }}</div>
                                            <div class="review-rating">
                                                @for($i = 0; $i < $rating->rating; $i++)
                                                    <x-icon name="star" class="icon-inline" />
                                                @endfor
                                                <span style="color: #64748B; margin-left: 0.5rem;">{{ $rating->rating }}/5</span>
                                            </div>
                                        </div>
                                    </div>
                                    @if($rating->review)
                    ...
                    @else
                        <div class="no-reviews">
                            <p style="font-size: 1.1rem; margin-bottom: 1rem;">
                                <x-icon name="info" class="icon-inline" />Belum ada ulasan untuk buku ini
                            </p>
    ...
    <div style="background: white; padding: 2rem; border-radius: 16px; max-width: 500px; width: 90%; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3); position: relative;">
        ...
        <form method="POST" action="{{ route('loans.store') }}">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="text" value="{{ $book->title }}" readonly style="width:100%; padding:0.75rem; border-radius:8px; border:1px solid #E0E7FF; background:#F3F4F6; margin-bottom:0.5rem;">
            </div>
            <div style="margin-bottom: 1.5rem;">
            ...
            <div style="display: flex; gap: 1rem;">
                ...
                <button type="submit" style="flex: 1; padding: 0.75rem; background: #2563EB; color: white; border: none; border-radius: 8px; font-weight: 700; cursor: pointer;">
                    <x-icon name="check-circle" class="icon-inline" />Pinjam Sekarang
                </button>
            </div>
        </form>
    </div>
...
<script>
    ...
    function confirmBorrow() {
        ...
        window.location.href = "{{ Auth::check() ? route('loans.store') : route('login') }}";
    }
